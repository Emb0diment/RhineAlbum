...
let albumPlayer: AlbumPlayer | undefined;
...
$("#stage").innerHTML = `
  ...
  <nav class="system-nav" aria-label="系统导航"><button data-player="import">＋ 导入专辑</button>
    <button data-action="search"><span class="nav-glyph">⌕</span> ALBUM INDEX <span class="key">/</span></button>
    <button data-action="saved" aria-label="查看收藏专辑" title="收藏专辑">＋ SAVED <span id="saved-count">00</span></button>
  ...
  <p id="skip" class="skip">点击任意位置进入</p>
  <section id="boot" class="boot" aria-label="系统启动">
    ...
    <div class="welcome"><div class="welcome-panel"></div><div class="welcome-heading">WELCOME TO</div><div class="welcome-company"><strong>RHINE LAB.LLC.</strong><strong class="welcome-highlight" aria-hidden="true">RHINE LAB.LLC.</strong></div><div class="welcome-database">AUDIO ARCHIVE</div><div class="welcome-logo">${logo}</div></div>
  </section>
  ...
  <section id="archive-ui" class="archive-ui" aria-label="专辑选择">
    <div class="archive-callout"><div class="eyebrow">AUDIO ARCHIVE <span>／</span> <span id="archive-category">机构专辑</span></div><button class="file-title" data-action="open">ALBUM NUMBER: <span id="selected-id">X-<span id="selected-code">001</span></span><span class="file-open">↗</span></button><div class="callout-rule"><i></i></div><div class="file-summary"><span id="selected-title">莱茵生命</span><span id="selected-clearance">BUSINESS AREA</span></div><button class="read-file" data-action="open">OPEN ALBUM <span>→</span></button></div>
    <div id="hover-label" class="hover-label" hidden>X-<span id="hover-code">001</span> / <span id="hover-title"></span></div>
    <div class="archive-counter"><span class="tiny-label">ALBUM / SELECT</span><div><span id="selected-number">01</span><i>/</i><span class="count-total">12</span></div></div>
    <div class="archive-navigation"><button data-action="prev" aria-label="上一个专辑">↑</button><div id="file-ticks" class="file-ticks"></div><button data-action="next" aria-label="下一个专辑">↓</button></div>
    <div class="column-navigation"><button data-action="column-prev" aria-label="上一张歌单">←</button><div><span id="column-number">PLAYLIST <span id="column-index">01</span> / <span id="playlist-total">01</span></span><strong id="column-name">机构专辑</strong></div><button data-action="column-next" aria-label="下一张歌单">→</button></div>
    <div class="archive-hint"><kbd>←</kbd> <kbd>→</kbd> 切换列 <span>／</span> <kbd>↑</kbd> <kbd>↓</kbd> 前后专辑 <span>／</span> <kbd>ENTER</kbd> 读取</div>
  </section>
  <section id="detail-ui" class="detail-ui" aria-label="专辑内容" hidden>
    <button class="back-button" data-action="back">← <span>ALBUM OVERVIEW</span><small>ESC</small></button>
    <div class="object-caption"><span id="object-id">NO.001</span><div>AUDIO ARCHIVE</div><small>DRAG TO INSPECT <span>↔</span></small><button class="viewer-open" data-action="model-viewer">360° 查看专辑模型 <span>↗</span></button></div>
    <article id="detail-content" class="detail-content"></article>
  </section>
  <div class="powered">POWERED BY <b>RHINE LAB</b><i></i></div>
  <footer class="system-footer"><span><i class="status-light"></i> SESSION AUTHORIZED<button type="button" class="three-toggle" data-action="toggle-three" aria-pressed="true" title="卸载三维模型，保留音乐播放">3D 开启</button></span><span>JOYCE MOORE <i>／</i> <span id="clock">00:00:00</span></span><button data-action="replay" title="重播启动流程">REINITIALIZE ↗</button></footer>
  ...
  <div id="loading" class="loading"><div class="loading-mark">${logo}</div><span>CONNECTING TO AUDIO ARCHIVE</span><i></i></div>
`;
...
$("#viewport").insertAdjacentHTML("beforeend", '<p class="mobile-entry">点击任意位置进入专辑</p>');
...
let lastSceneFrame = 0, lastInteraction = performance.now();
let autoQualityStep = 0, slowWindows = 0;
let autoQuality = readLocal<boolean>('rhine-auto-quality', true);
for (const event of ['pointerdown', 'keydown', 'wheel']) document.addEventListener(event, () => { lastInteraction = performance.now(); }, { passive: true });
function savePrefs() {
  saveAudioPrefs();
  if (prefs.reduced) {
...
function setMode(next: Mode) {
  ...
  if (next !== "boot") {
    bootSequence.reset();
    $(".file-title").firstChild!.textContent = "ALBUM NUMBER: ";
...
function select(index: number, navigation?: ArchiveNavigation, keepDetail = false) {
  if(albumPlayer&&!albumPlayer.canSelect((index+records.length)%records.length))return;
  selected = (index + records.length) % records.length;
  columnMemory[fileLocation(selected).lane] = selected;
  if (mode === "detail" && !keepDetail) setMode("archive");
  activeTab = "overview";
  scene?.select(selected, navigation);
  updateSelection(navigation);
  if (keepDetail && mode === "detail") renderDetail();
...
function stepFile(direction: number) {
  if(albumPlayer){select(albumPlayer.stepTrack(direction));return;}
...
function stepColumn(direction: number) {
  if(albumPlayer){if(mode==="detail")setMode("archive");albumPlayer.switchPlaylist(direction);scene?.select(selected);updateSelection();return;}
...
function updateSelection(navigation?: ArchiveNavigation) {
  ...
  fileTicks.forEach((button, slot) => {
    ...
    button.setAttribute("aria-label", `选择专辑 ${record.id} ${record.title}`);
    button.title = `${record.id} · ${record.title}`;
    button.classList.toggle("selected", index === selected);
    button.setAttribute("aria-pressed", String(index === selected));
  });
  $("#saved-count").textContent = String(saved.size).padStart(2, "0");
  if(albumPlayer){const n=albumPlayer.navigation();columnTitle.update({text:n.title,animated:!prefs.reduced});categoryTitle.update({text:n.title,animated:!prefs.reduced});columnCounter.update({value:n.index,animated:!prefs.reduced});$("#playlist-total").textContent=String(n.count).padStart(2,'0');fileCounter.update({value:n.track,animated:!prefs.reduced});$(".count-total").textContent=String(n.tracks).padStart(2,'0');}
}
...
function toggleSaved() {
  const id = albumPlayer?.key(selected) ?? records[selected].id;
  ...
  button.firstChild!.textContent = added ? "− REMOVE FROM SAVED" : "＋ SAVE ALBUM";
  button.querySelector("span")!.textContent = added ? "已收藏" : "收藏专辑";
  ...
  audio.play("confirm");
  notify(saved.has(id) ? "专辑已加入收藏" : "已取消收藏");
}
function renderDetail() {
  ...
  <dl class="metadata"><div><dt>ARTIST / 艺术家</dt><dd>${escapeHtml(r.department)}</dd></div><div><dt>COLLECTION / 曲目</dt><dd>${escapeHtml(r.date)}</dd></div><div><dt>CREDIT / 署名</dt><dd>${escapeHtml(r.lead)}</dd></div><div><dt>STATUS / 状态</dt><dd><i></i>${r.clearance === "RESTRICTED" ? "目录访问" : "已收录 · 可播放"}</dd></div></dl>
  <div class="detail-tabs" role="tablist"><button id="tab-overview" class="active" role="tab" aria-controls="tab-panel" aria-selected="true" data-tab="overview">01 <span>曲目</span></button><button id="tab-notes" role="tab" aria-controls="tab-panel" aria-selected="false" data-tab="notes">02 <span>专辑信息</span></button><button id="tab-history" role="tab" aria-controls="tab-panel" aria-selected="false" data-tab="history">03 <span>播放记录</span></button><i class="tab-indicator" aria-hidden="true"></i></div>
  <div id="tab-panel" class="tab-panel" role="tabpanel">${overview()}</div>
  <div class="detail-actions"><button class="solid-button" data-action="bookmark">${saved.has(albumPlayer?.key(selected) ?? r.id) ? "− REMOVE FROM SAVED" : "＋ SAVE ARCHIVE"}<span>${saved.has(albumPlayer?.key(selected) ?? r.id) ? "已收藏" : "收藏专辑"}</span></button><button class="export-button" data-player="import">IMPORT <span>＋</span></button></div>
  <div class="detail-footnote"><a href="${escapeHtml(r.source)}" target="_blank" rel="noopener">原项目 ↗</a><span>${String(selected + 1).padStart(3, "0")} / ${String(records.length).padStart(3, "0")}</span></div>`;
  $("#detail-content").setAttribute("tabindex", "-1");
  $('[data-action="bookmark"]').setAttribute("aria-pressed", String(saved.has(albumPlayer?.key(selected) ?? r.id)));
  documentDecryption.reset($("#detail-content"), prefs.reduced || !scene || scene.decryptionFrame.phase === "clear");
  setTab(activeTab, false);
}
function overview() {
  if (albumPlayer) return albumPlayer.panel(selected);
...
function setTab(tab: string, sound = true) {
  ...
  $("#tab-panel").innerHTML = tab === "overview" ? overview() : tab === "notes" ? albumPlayer?.info(selected) ?? "" : albumPlayer?.log() ?? "";
...
function renderModal() {
  ...
  $("#modal-root").innerHTML =
    `<div class="modal-backdrop"><section class="terminal-modal ${modal === "settings" ? "settings-modal" : ""}" role="dialog" aria-modal="true" aria-label="${modal === "settings" ? "系统设置" : modal === "saved" ? "收藏专辑" : "专辑检索"}"><div class="modal-top"><span>RHINE LAB / ${modal === "settings" ? "SYSTEM PREFERENCES" : "ARCHIVE DIRECTORY"}</span><button data-action="close-modal" aria-label="关闭窗口">CLOSE <span>×</span></button></div>${modal === "settings" ? settingsMarkup() : `<h2>${modal === "saved" ? "SAVED ARCHIVES" : "ARCHIVE INDEX"}<small>${modal === "saved" ? "收藏专辑" : "内部专辑检索"}</small></h2><div class="search-field"><span>⌕</span><input id="archive-search" type="search" autocomplete="off" placeholder="输入专辑编号、名称或科室" aria-label="检索专辑"/><span class="key">ESC</span></div><div class="category-filters">${(albumPlayer ? ["全部专辑", ...albumPlayer.playlistNames()] : categories).map((c, i) => `<button data-filter="${escapeHtml(c)}" class="${i === 0 ? "active" : ""}">${escapeHtml(c)}</button>`).join("")}</div><div class="result-header"><span>FILE / 专辑</span><span>DEPARTMENT / 科室</span><span>ACCESS</span></div><div id="search-results" class="search-results"></div><div class="modal-bottom"><span id="result-count"></span><span>AUDIO ARCHIVE <i>●</i> CONNECTED</span></div>`}</section></div>`;
...
function renderResults() {
  if (albumPlayer) {
    const results=albumPlayer.catalog().filter(r=>(modal!=="saved"||saved.has(r.key))&&(filter==="全部专辑"||r.album===filter)&&[r.title,r.album,r.artist].join(' ').toLowerCase().includes(searchQuery.toLowerCase()));
    $("#search-results").innerHTML=results.length ? results.map(r=>`<button class="result-row" data-result="${r.key}"><span class="result-name"><b>${String(r.number).padStart(2,'0')}</b><span>${escapeHtml(r.title)}<small>${escapeHtml(r.album)}</small></span>${saved.has(r.key)?'<i>＋</i>':''}</span><span>${escapeHtml(r.artist)}</span><span>↗</span></button>`).join('') : '<div class="empty-results"><strong>没有匹配的歌曲</strong><p>可切换歌单分类，或输入其他歌名、歌手。</p></div>';
    $("#result-count").textContent=results.length+' 首歌曲';return;
  }
  const results = records
    .map((r, i) => ({ r, i }))
    .filter(
      ({ r }) =>
        (modal !== "saved" || saved.has(albumPlayer?.key(selected) ?? r.id)) &&
        (filter === "全部专辑" || r.category === filter) &&
  ...
  $("#search-results").innerHTML = results.length
    ? results
        .map(
          ({ r, i }) =>
            `<button class="result-row" data-result="${i}"><span class="result-name"><b>${r.id}</b><span>${escapeHtml(r.title)}<small>${escapeHtml(r.en)}</small></span>${saved.has(albumPlayer?.key(selected) ?? r.id) ? "<i>＋</i>" : ""}</span><span>${escapeHtml(r.department)}</span><span>${r.clearance === "RESTRICTED" ? "CATALOG ONLY" : "AUTHORIZED"} <i>↗</i></span></button>`,
        )
        .join("")
    : `<div class="empty-results"><span>∅</span><strong>${modal === "saved" && !searchQuery ? "尚无收藏专辑" : "没有匹配的专辑"}</strong><p>${modal === "saved" && !searchQuery ? "读取专辑时，选择 SAVE ARCHIVE 将其保存在此处。" : "尝试其他名称、专辑编号，或切换科室分类。"}</p><button data-action="reset-search">${modal === "saved" ? "查看全部专辑 →" : "重置检索 →"}</button></div>`;
...
function settingsMarkup() {
  return `<h2>SYSTEM SETTINGS<small>终端偏好设置</small></h2><p class="settings-intro">JOYCE MOORE <span>·</span> SESSION AUTHORIZED</p>${isWallpaper ? '<p class="wallpaper-settings-note">每次启动都会读取 Wallpaper Engine 中的设置。在此修改仅对当前运行生效，无法持久保存；如需保留，请在 Wallpaper Engine 的壁纸属性中调整。</p>' : ""}<div class="settings-list"><label><div><strong>自动优化</strong><span>帧率持续偏低时降低渲染分辨率和后期效果，保留模型与动画</span></div><input type="checkbox" id="auto-quality" ${autoQuality ? "checked" : ""}/><i class="toggle"></i></label>${themeSettingsMarkup(prefs.colorTheme === "dark")}${!isWallpaper ? `<label><div><strong>SUPER PERFORMANCE</strong><span>降低三维画质和渲染分辨率，保留完整动效；关闭后恢复原画质</span></div><input type="checkbox" data-pref="superPerformance" ${prefs.superPerformance ? "checked" : ""}/><i class="toggle"></i></label>` : ""}${workbench?.settingsMarkup() ?? ""}${audioSettingsMarkup(prefs)}<label><div><strong>REDUCED MOTION</strong><span>跳过开机动画，简化选档、镜头和文字动效</span></div><input type="checkbox" data-pref="reduced" ${prefs.reduced ? "checked" : ""}/><i class="toggle"></i></label></div>${motionSettingsMarkup()}${qualityMarkup(prefs.rendering)}${pwaSettingsMarkup()}<div class="settings-shortcuts">${isWallpaper ? '<span>DESKTOP CONTROLS</span><p>拖动阵列或点击界面按钮浏览专辑。桌面模式下，方向键与滚轮可能无法传入壁纸。</p>' : '<span>KEYBOARD CONTROLS</span><p><kbd>←</kbd><kbd>→</kbd> 切列 <kbd>↑</kbd><kbd>↓</kbd> 选档 <kbd>ENTER</kbd> 读取 <kbd>/</kbd> 检索 <kbd>ESC</kbd> 返回</p>'}</div><div class="settings-bottom">${!isWallpaper && document.fullscreenEnabled ? '<button data-action="fullscreen">FULLSCREEN <span>↗</span></button>' : ''}<button data-action="restart">REINITIALIZE SYSTEM <span>↻</span></button></div><div class="modal-bottom"><span>ANALYSIS OS / 1.0 · 使用 MiSans 字体（小米） <a href="${assetUrl("fonts/MiSans-license.pdf")}" target="_blank" rel="noopener">字体许可</a></span><span>POWERED BY RHINE LAB</span></div>`;
}
...
document.addEventListener('click',e=>{if(started&&ready&&mode==='boot'&&!modal&&!document.querySelector('dialog[open]')){e.preventDefault();e.stopImmediatePropagation();setMode('archive');audio.play('confirm');}},true);
document.addEventListener("click", (e) => {
  ...
  if (el.dataset.result?.includes(':') && albumPlayer) { const key=el.dataset.result;closeModal(()=>albumPlayer?.openKey(key));return; }
...
function frame(ms: number) {
  ...
  const sourceDialog = Boolean(document.querySelector('dialog[open]'));
  const interval = mode === 'boot' || mode === 'detail' || ms - lastInteraction < 3500 ? 0 : prefs.reduced ? 1000 / 15 : 1000 / 30;
  if (!viewer?.isOpen && !sourceDialog && (!cinema || cinema.time >= 21.9) && ms - lastSceneFrame >= interval) { scene?.update(time, cinema); lastSceneFrame = ms; }
  viewer?.update(time);
  ...
  if (ms - frameStart > 1000) {
    ...
    if (autoQuality && scene && !sourceDialog && mode !== 'boot' && !prefs.superPerformance && ms > 12000) {
      slowWindows = fps < 22 ? slowWindows + 1 : 0;
      if (slowWindows >= 3 && autoQualityStep < 2) {
        autoQualityStep++; slowWindows = 0;
        const scale = autoQualityStep === 1 ? 75 : 55;
        scene.setQuality({ ...prefs.rendering, scale: Math.min(scale, prefs.rendering.scale), pixelRatio: Math.min(1, prefs.rendering.pixelRatio), aoSamples: 0, depthOfField: 0, transmission: Math.min(.5, prefs.rendering.transmission) });
        $("#stage").dataset.autoQuality = String(autoQualityStep);
        if (autoQualityStep === 1) notify('已自动降低渲染负担，保留玻璃阵列与动画。');
      }
    }
...
function bindScene(scene: ArchiveScene, cell?: { lane: number; row: number }) {
    scene.playlistRows = true;
    scene.onDoubleSelect = (i, cell) => {
      if(mode!=="archive"||modal||viewer?.isOpen||!albumPlayer?.canSelect(i))return;
      select(i,cell?{cell}:undefined);setMode("detail");albumPlayer.playSelected();
    };
    scene.select(selected, cell ? { cell } : undefined);
    scene.onSelect = (i, cell) => {
      if (mode !== "archive" || modal || viewer?.isOpen) return;
      const previous=scene.selectionPosition;
      if(cell&&cell.lane===previous.lane){const pages=Math.floor((cell.row-12)/8)-Math.floor((previous.row-12)/8);if(pages)albumPlayer?.moveRowPage(i,pages);}
...
async function start() {
  try {
    ...
    await Promise.all([
      ...
      document.fonts.load("300 20px MiSans", "ACCESS WELCOME TO AUDIO ARCHIVE"),
      document.fonts.load("400 20px MiSans", "身份信息确认请求已接收开始处理权限验证通过欢迎访问莱茵生命内部资料专辑编号保密级别商业区选择专辑：0123456789 JOYCE MOORE"),
      document.fonts.load("600 20px MiSans", "SYNTHESIZE INFORMATION ANALYSIS OS"),
      document.fonts.load("700 20px MiSans", "RHINE LAB WELCOME TO AUDIO ARCHIVE"),
    ]);
    ...
    console.error(error);
    $("#loading").innerHTML =
      '<div class="error-state"><strong>CONNECTION INTERRUPTED</strong><p>三维专辑资源未能载入。请确认浏览器已启用硬件加速，然后重新连接。</p><button onclick="location.reload()">RECONNECT →</button></div>';
  }
}
function completeStartup(silent: boolean) {
  ...
  setTimeout(() => {
    ...
    if (entry && restoreFocus) {
      const target = mode === "boot" ? $("#stage") : $(".read-file");
      if(mode==="boot")target.tabIndex=-1;
...
albumPlayer = new AlbumPlayer({ browse: index => select(index, undefined, true), selected: () => selected, changed: () => { updateSelection(); if (mode === "detail") renderDetail(); }, suppress: playing => { musicSuppressed = playing; configureAudio(); }, notify, open: index => { if (!ready) return; select(index); setMode("detail"); } });
import { NovelPlayer } from './novel-player';
new NovelPlayer(albumPlayer.audio, notify);

document.addEventListener("change", e => { if ((e.target as HTMLElement).id !== "auto-quality") return; autoQuality = (e.target as HTMLInputElement).checked; autoQualityStep = 0; slowWindows = 0; try { localStorage.setItem("rhine-auto-quality", JSON.stringify(autoQuality)); } catch {} scene?.setQuality(effectiveRenderQuality()); });
