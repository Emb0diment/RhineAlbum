...
import { AlbumCovers } from './album-covers';
...
export class ArchiveScene {
  ...
  playlistRows = false;
  get selectionPosition(){return {...this.selectedCell};}
  onDoubleSelect?: (index: number, cell?: ArchiveCell) => void;
  private lastSlotTap?: { index: number; cell: ArchiveCell; time: number; x: number; y: number };
  ...
  async load(assetUrl = publicAsset("assets/archive-cassette.glb")) {
    ...
    this.covers = new AlbumCovers(this.scene, this.inputEvents.signal);
    this.model.remove(label);
    this.model.add(this.covers.face, label);
    this.drawLabel(0);
  ...
  async createAssemblyModel() {
    ...
    model.add(label);
    meshes.push(label);
    let coverTexture: THREE.Texture | undefined;
    if (this.covers) {
      const original = this.covers.face;
      coverTexture = (original.material as THREE.MeshBasicMaterial).map!.clone();
      coverTexture.needsUpdate = true;
      const cover = new THREE.Mesh(original.geometry.clone(), new THREE.MeshBasicMaterial({ map: coverTexture, toneMapped: false, side: THREE.DoubleSide }));
      cover.userData.assemblyPart = 'cover';
      model.add(cover); meshes.push(cover);
    }
    return {
      model,
  ...
  select(index: number, navigation?: ArchiveNavigation) {
    ...
    if (this.looping && changed && this.loaded && this.lift.value > 0.0001) {
      ...
      for (const child of group.children) if (child.userData.albumCover) {
    ...
    canvas.addEventListener("pointerup", (e) => {
      ...
      if (!cancelled && browse && this.canBrowse()) {
        ...
        if (this.archiveDrag.active) {
          ...
          const cell = this.pickCell(e.clientX, e.clientY);
          const last=this.lastSlotTap;
          // The first tap lifts the cassette and moves the camera. Keep its hit
          // for the second tap at the same point, even if the ray now hits behind it.
          const twice=last&&e.timeStamp-last.time<450&&Math.hypot(e.clientX-last.x,e.clientY-last.y)<24;
          if(twice){this.lastSlotTap=undefined;this.onDoubleSelect?.(last.index,last.cell);}
          else if(cell){const index=fileAtCell(cell);this.lastSlotTap={index,cell:{...cell},time:e.timeStamp,x:e.clientX,y:e.clientY};this.onSelect?.(index,cell);}
        }
      }
      reset();
    }, { signal: this.inputEvents.signal });
  ...
  update(
    ...
    // Closed-form damping stays stable for a longer frame; avoid slowing every
    // extraction to a crawl on a busy GPU. Hidden tabs still stop in main.ts.
    const dt = Math.min(elapsed, 0.15);
    this.last = time;
    this.clock = time;
    ...
    for (const cell of this.cells) {
      const { row, lane } = cell;
      // Each playlist page contains eight ordered slots. Do not paint another
      // modulo copy beside it: it would appear to restart the playlist mid-row.
      if(this.playlistRows&&!fixed){const first=12+Math.floor((this.selectedCell.row-12)/8)*8;if(row<first||row>=first+8)continue;}
      ...
      this.covers?.setInstance(i, this.dummy.matrix, fileAtCell(cell));
    }
    this.covers?.finish(this.drawnCells.length);
    for (const inst of this.instances) {
